import requests, os, json, repltalk, asyncio, getpass, aiohttp
...
class assets(): 
	async def replit_repls(category=None):
		if category is None:
			raise Exception('Fill in the category parameter!')
		else:
			c = repltalk.User()
			return await c.get_repls(limit=None, True)
